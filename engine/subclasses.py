"""Специализации и подклассы героев (Ascendancy).

Каталог подклассов общий для обоих стеков: `core/subclasses.py`
реэкспортирует его отсюда. Специализация открывается с
`SUBCLASS_MIN_LEVEL` — число тоже одно на два стека.
"""

SUBCLASS_MIN_LEVEL = 10     # с какого уровня можно выбрать специализацию
...
def choose_subclass(character, subclass_key: str) -> dict:
    """Выбор специализации подкласса."""
    if (character.level or 1) < SUBCLASS_MIN_LEVEL:
        return {"ok": False,
                "reason": f"Специализация доступна только с "
                          f"{SUBCLASS_MIN_LEVEL}-го уровня!"}
...
def subclass_bonuses(character) -> dict:
