...
Теперь у мира три силы с противоположными интересами, и выбор стороны
что-то значит.
...
Помощь одной злит противоположную (`RIVALS`), поэтому нельзя быть своим
...
FACTIONS = {
    "guard": ("🛡", "Стража Погоста",
              "Пока стоит частокол — стоит и деревня.", "cult"),
    "scavengers": ("💰", "Гильдия падальщиков",
                   "Мёртвым золото ни к чему.", "guard"),
    "cult": ("🌑", "Культ Пожирателя",
             "Всё кончится. Мы лишь торопим неизбежное.", "guard"),
}
ORDER = ["guard", "scavengers", "cult"]
...
# За что дают репутацию: событие -> {фракция: сколько}
DEEDS = {
    "undead_slain": {"guard": 2, "cult": -2},        # упокоил нежить
    "beast_slain": {"guard": 1},                     # убил зверя
    "grave_looted": {"scavengers": 4, "guard": -3},  # обчистил чужую могилу
    "chest_opened": {"scavengers": 1},               # вскрыл сундук
    "quest_done": {"guard": 3},                      # выполнил задание жителя
    "boss_slain": {"guard": 5, "cult": 3},           # свалил мирового босса
