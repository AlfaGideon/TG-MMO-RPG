...
def calculate_gear_score(character):
    ...
    gs = 0
    gs += getattr(character, "gear_score", 0) or 0
    gs += int((getattr(character, "strength", 0) or 0) * 0.8)
    gs += int((getattr(character, "agility", 0) or 0) * 0.7)
    gs += int((getattr(character, "dexterity", 0) or 0) * 0.9)
    gs += int((getattr(character, "intelligence", 0) or 0) * 0.6)
    gs += int((getattr(character, "crit_chance", 0) or 0) * 2)
    gs += int((getattr(character, "crit_damage", 0) or 0) * 0.8)
    gs += int((getattr(character, "life_on_hit", 0) or 0) * 3)
    gs += int((getattr(character, "thorns_damage", 0) or 0) * 2)
    return max(0, gs)
