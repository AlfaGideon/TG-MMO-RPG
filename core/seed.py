...
async def seal_fortress_and_link_gates(session):
    """Восстановить входы в центральную крепость после пересшивки мира.

    ``relink_all`` намеренно удаляет все межлокационные переходы перед
    построением обычных швов. Поэтому четыре диагональных портала ворот в
    центральные клетки цитадели нельзя считать одноразовой частью сида: их
    нужно вызывать после *каждой* пересшивки (перетаскивание, перемешивание
    или кнопка «Пересобрать переходы» в панели).

    Ортогональные двери в цитадель не допускаются: вход только через четыре
    соответствующие воротные локации.
    """
...
async def seed_database():
    ...
    async with async_session() as session:
        ...
        if result.scalars().first():
            ...
            # Ранние версии и ручная «пересборка переходов» могли стереть
            # четыре диагональных входа в цитадель. Восстанавливаем их и в
            # уже существующей базе, в том числе при запуске без админки.
