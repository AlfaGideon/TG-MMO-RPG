...
## 2026-07-31 — деньги-разряды и переработка выбора локации

**Валюты (`engine/money.py` ↔ `core/money.py`).** Кошелёк героя хранится
**одним числом в бронзе** (`Player.gold` / `Character.gold`), разряды
🥉/🥈/🥇 считаются только при выводе. Три отдельных счётчика заводить
нельзя: тогда каждое начисление обязано само переносить сотни между
разрядами, и одна забытая награда даёт «100🥈» вместо золотого.

- Курс: 100🥉 = 1🥈, 100🥈 = 1🥇 (10 000🥉). Числа общие для стеков,
  совпадение стережёт `tests/test_parity.py`.
- Все операции с деньгами — через `money.earn/pay/can_pay/lack`, а не
  `p.gold += ...`: иначе кошелёк уходит в минус, а «не хватает»
  считается по-разному в лавке, кузнице и на аукционе.
- Вывод — только `money.fmt()` (полная запись), `money.short()`
  (кнопки), `money.plus()` (журнал боя). Голый `p.gold` в тексте
  печатать не нужно — это сумма в бронзе, игроку она ни о чём не говорит.
- 💎 премиум (`Player.premium`, колонка `characters.premium` + миграция)
  — отдельный счётчик, из мира не выпадает и в могиле не теряется.
  Обмен **односторонний**: 💎 → монеты по курсу `premium_rate`. Обратный
  обмен не добавлять — это превратит донат в вывод средств.
- `welcome_bonus` теперь реально применяется в `Store.player()`; раньше
  настройка была, но новичок всегда получал зашитые в модель 50.
- В Jinja-шаблонах админки есть фильтры `| coins` и `| coins_short`.

**Карта локаций (`webapp/pages/world_map.py`).** Кисть и переключатель
тумана войны удалены: кисть конфликтовала с редактором клетки и работала
через раз, туман глазами игрока ничего не менял. Клик по клетке открывает
её в боковом доке. Локации переключаются крестом стрелок ▲▼◀▶ по
соседям с сетки мира (`neighbours()`); локация вне сетки листается
◀▶ по порядку списка, иначе до неё не добраться. Карта во флекс-строке
с переключателем этажей обязана иметь **явную ширину** — грид внутри
флекса ужимается по содержимому, из-за чего карта была вдвое мельче
эталонной из «Подземелий» (`tests/test_admin_layout.py` это стережёт).
