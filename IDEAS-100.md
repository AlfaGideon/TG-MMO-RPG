...
|---|---|---|---|---|
| 1. Тесты и CI | 10 | **все 10** | — | **раздел закрыт**: 12 забытых наборов, ложные провалы, флаки, нет CI, рассинхрон бандла |
| 2. Паритет `core/` → `engine/` | 25 | **все 25** | — | **раздел закрыт: 49 из 49 механик в обоих стеках** |
| 3. Вход игроку в боте | 25 | № 36–57 (кроме 58) | № 59 | подсистемы без единого вызова из `bot/handlers/` |
| 4. Админка `admin/main.py` | 10 | № 61–66 | — | остаток: № 67–70 (арена, знамения, престиж в карточке, realtime-лента) |
| 5. Панель `webapp/pages/` | 6 | **все 6** | — | **раздел закрыт**: карма, питомцы, бестиарий, археология, ломбард в Pyodide-панели |
| 6. Баги и долги из аудита | 12 | № 77–85, 87, 88 (все) | — | **все долги аудита закрыты** |
| 7. Трёхвалютная экономика | 6 | № 89–94 (все) | — | **долг закрыт: паритет 49/49** |
| 8. Документация | 6 | **все 6** | — | **раздел закрыт**: README, `IDEAS-next.md`, `AUDIT-BUGS.md` приведены к факту кода |
| **Итого** | **100** | **93** | **1** | — |

**Сделано на 2026-08-16: 93 пункта закрыто полностью, 1 частично (№ 59), 1 снят как задача-фантом (№ 58).**
Открытыми остались только № 20 и № 67–70 (остаток админки) — и № 59, ждущий решения владельца.
**Паритет: 49 из 49 механик в обоих стеках.** Разделы 1, 2, 5, 6, 7, 8 закрыты целиком.
Проверка: `python3 tests/run_all.py` → **71 набор, все зелёные** (было 41 запускаемый из 53 существующих в начале работ).

**Что добавила последняя партия (2026-08-16):**
* № 4 — `tests/_deps.py`: единый грейсфул-скип, 17 наборов вместо 11 из исходного списка; поддержка `httpx|httpx2`;
* № 5 — `tests/_seed.py`: `pin()` для игрового рандома и `unique_id()` мимо seed для UNIQUE-колонок БД;
* № 71, 73–76 — Pyodide-панель: карма и атлас в карточке игрока, вкладки «Питомцы», «Бестиарий», «Теневая экономика», блок археологии и метка тайника 🏺 на карте мира;
* № 96, 97, 99 — README (68 модулей `engine/`, 70 наборов, таблица команд) и `IDEAS-next.md` (статусы 1–10 по факту кода);
* новые наборы: `tests/test_suite_hygiene.py`, `tests/test_panel_subsystems.py`.
...
1. **№ 67–70** — остаток админки: арена и дуэли на `/battles`, редактор знамений, престиж/титулы/таланты в карточке игрока, realtime-лента событий новых подсистем. Это последний незакрытый блок из ста пунктов.
2. **№ 20** — питомцы (`PetTemplate`) в Pyodide-панели: единственный оставшийся пункт Раздела 1–3 помимо № 59.
3. **№ 59** — решение владельца по `core/auction_bid.py`: удалить модуль или довести торги со ставками (план из 4 шагов в его docstring, включая атомарный UPDATE против гонки ставок).
4. **Из `IDEAS-next.md`** — пробелы, которых в этих ста пунктах нет: прямой обмен между игроками (⬜), дом героя (⬜), игровые сезоны и планировщик событий (🟡 — сейчас только сезонная картинка), персональные уведомления (🟡).
