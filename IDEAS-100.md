...
**Сделано на 2026-08-14: 32 пункта закрыто полностью, 9 частично.**
Закрыты: № 1, 2, 3, 6, 7, 8, 9, 10, 36–47, 50, 61, 62, 63, 77, 78, 85, 88, 89, 95, 100.
Частично: № 35, 48, 49, 64, 65, 66, 72, 92 (остаток описан внутри каждого пункта).
Проверка: `python3 tests/run_all.py` → **57 наборов, все зелёные** (было 41 запускаемый из 53 существующих). Новые наборы: `tests/test_karma_omens_engine.py` (42 проверки), `tests/test_bot_entrypoints.py` (10 тестов), `tests/test_admin_subsystems.py` (7 тестов), `tests/test_bundle_integrity.py`.

### Что брать следующим (по стоимости/эффекту)
1. **№ 51–58** — оставшиеся входы в боте: гильдии, наставничество, дуэли, престиж, бестиарий, питомцы. Гильдии особенно: модели и логика есть, админка их уже показывает, но игрок не может создать ни одной.
2. **№ 79–80 (долг A)** — рефакторинг инвентаря на uid: чинит семейство багов с дубликатами вещей.
3. **№ 90–94** — добить трёхвалютную экономику и снять последний долг паритета.
4. **№ 67–70** — остаток админки: арена/дуэли, редактор знамений, престиж и титулы в карточке, realtime-лента новых событий.
