...
**Остаток:** дуэль идёт без согласия второй стороны (он узнаёт постфактум) — для подтверждения нужен обмен сообщениями между игроками.
...
**Статус:** 🟡 частично. **Сделано:** блок «🕯 Чёрный рынок» на `/editor/subsystems` — витрина из `core/blackmarket.get_black_market_wares()` (товар, цена, эффект) с пометкой, что ассортимент задаётся в коде.
**Остаток:** добавление/снятие лотов из админки и источник лота (залог/админ) — появится вместе с перетеканием просроченных залогов на витрину (№ 48).
**Файлы:** `admin/main.py`, `admin/templates/editor_subsystems.html`.
...
**Статус:** 🟡 частично. **Сделано:** блок «🌙 Фаза луны» на `/editor/subsystems` — текущая фаза с описанием и таблица всех четырёх фаз с множителями (`mob_mult`, `material_mult`, `magic_dark_mult`); текущая выделена.
**Остаток:** принудительная смена фазы невозможна by design — фаза чисто вычисляется от времени (`get_current_lunar_phase`). Чтобы дать кнопку, нужно сперва хранить override в `AppSetting` и читать его в `core/lunar.py`.
**Файлы:** `admin/main.py`, `admin/templates/editor_subsystems.html`.

### № 66. Админка: гильдии и наставничество
**Статус:** 🟡 частично. **Сделано:** блок «🏛 Гильдии» на `/editor/subsystems` — список гильдий (название, уровень, казна, число участников). Модели `Guild`/`GuildVaultItem` объявлены не в `core/models.py`, а прямо в `core/guilds.py`, поэтому выборка обёрнута в `try/except`: пока миграция не создала таблицы, блок показывает подсказку вместо ошибки 500.
**Остаток:** роспуск гильдии и таблица наставник–ученик — после того, как у гильдий появится вход для игроков (№ 51–52); сейчас данных попросту нет.
**Файлы:** `admin/main.py`, `admin/templates/editor_subsystems.html`.
...
| 3. Вход игроку в боте | 25 | № 36–47, 50, 51, 54, 55 | № 48, 49, 52, 53 | подсистемы без единого вызова из `bot/handlers/` |
| 4. Админка `admin/main.py` | 10 | № 61, 62, 63 | № 64, 65, 66 | 13 подсистем с нулём упоминаний в админке |
...
**Сделано на 2026-08-14: 35 пунктов закрыто полностью, 11 частично.**
Закрыты: № 1, 2, 3, 6, 7, 8, 9, 10, 36–47, 50, 51, 54, 55, 61, 62, 63, 77, 78, 85, 88, 89, 95, 100.
Частично: № 35, 48, 49, 52, 53, 64, 65, 66, 72, 92 (остаток описан внутри каждого пункта).
Проверка: `python3 tests/run_all.py` → **58 наборов, все зелёные** (было 41 запускаемый из 53 существующих). Новые наборы: `tests/test_karma_omens_engine.py` (42 проверки), `tests/test_bot_entrypoints.py` (10 тестов), `tests/test_admin_subsystems.py` (7 тестов), `tests/test_guild_bounty_duel_entrypoints.py` (10 тестов), `tests/test_bundle_integrity.py`.
...
1. **№ 56–58** — остаток входов в боте: иллюзии, бестиарий, питомцы (последние доступны только админке).
2. **№ 52, 53** — добить частичное: бонус наставника +25 % в `_finish_victory` и согласие второй стороны на дуэль.
