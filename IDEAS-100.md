...
|---|---|---|---|---|
| 1. Тесты и CI | 10 | **все 10** | — | **закрыт**: забытые наборы, ложные провалы, флаки, CI, рассинхрон бандла |
...
| 4. Админка `admin/main.py` | 10 | **все 10** | — | **закрыт**: подсистемы, арена и дуэли, знамения, эндгейм-прогресс, живая лента |
| 5. Панель `webapp/pages/` | 6 | **все 6** | — | **закрыт**: карма, питомцы, бестиарий, археология, ломбард в Pyodide-панели |
| 6. Баги и долги из аудита | 12 | № 77–85, 87, 88 (все) | — | **все долги аудита закрыты** |
| 7. Трёхвалютная экономика | 6 | № 89–94 (все) | — | **закрыт: паритет 50/50** |
| 8. Документация | 6 | **все 6** | — | **закрыт**: README, `IDEAS-next.md`, `AUDIT-BUGS.md` приведены к факту кода |
| **Итого** | **100** | **99** | **1** | — |

**Сделано на 2026-08-16: 99 пунктов закрыто полностью, 1 частично (№ 59), 1 снят как задача-фантом (№ 58).**
Открытым остался единственный пункт — **№ 59** (`core/auction_bid.py`), и он ждёт решения владельца: удалить модуль или довести торги со ставками.
**Паритет: 50 из 50 механик в обоих стеках.** Все восемь разделов, кроме третьего (где висит только № 59), закрыты целиком.
Проверка: `python3 tests/run_all.py` → **72 набора, все зелёные** (было 41 запускаемый из 53 существующих в начале работ).

**Что добавила последняя партия (2026-08-16, вторая):**
* № 67 — секции «Колизей Теней» и «Вызовы на дуэль» на `/battles`; попутно закрыт баг: вызов на дуэль висел вечно, теперь у него TTL 300 с;
* № 68 — страница `/editor/omens`: свои знамения в `AppSetting`, предпросмотр, таблица предвестий бедствий;
* № 69 — блок «🌌 Эндгейм-прогресс» в карточке игрока и сброс созвездий одной кнопкой;
* № 70 — пять новых типов событий (`karma_changed`, `pawn_loan`, `town_invested`, `dividends_paid`, `omen_shown`), публикация из самих механик, карточка «Живая лента мира» с фильтром;
* № 86 — знамение при живом бедствии показывает его предвестие: использовано поле `omen`, которое было в `KINDS` у каждого вида и не читалось нигде;
* № 20 — `engine/pets.py`: правила каталога шаблонов питомцев стали общими, в панели появилась вкладка «🥚 Шаблоны питомцев»;
* новый набор: `tests/test_admin_arena_omens.py`.
...
1. **№ 59** — единственный незакрытый пункт из ста: решение владельца по `core/auction_bid.py`. Либо удалить модуль (он не импортируется нигде, включая тесты), либо довести торги со ставками — план из 4 шагов лежит в его docstring, включая атомарный UPDATE против гонки ставок.
2. **Из `IDEAS-next.md`** — пробелы, которых в этих ста пунктах не было: прямой обмен между игроками (⬜ нет), дом героя (⬜ нет), игровые сезоны и планировщик событий (🟡 сейчас только сезонная картинка), персональные уведомления (🟡 есть только массовые рассылки).
3. **Браузерный паритет осады** — `core/worldevents.start_siege` и `FactionOutpost` живут только на сервере; в `engine/` осады нет (см. пункт 4 таблицы `IDEAS-next.md`).
