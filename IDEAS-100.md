...
**Статус:** ✅ сделано (2026-09-02). **Сделано:** кнопка «🏛 Летопись» в главном меню
→ экран `legends_hall` (`bot/handlers/world_extra.py`) на общей вёрстке
`engine/legends.hall_text` (имена героев теперь экранированы — имя было
ввод игрока). Запись первопроходств в проде: `core/worldevents.hit_boss`
пишет «Первое убийство: …» на добившего (победителя гонки), `_restore` —
веху «Первое пережитое бедствие: …» без владельца; повторная запись того
же ключа отклоняется, как и заведено в движке.
**Файлы:** `core/legends.py` (character опционален), `core/worldevents.py`,
`engine/legends.py`, `bot/handlers/world_extra.py`, `bot/keyboards/inline.py`.
**Проверка:** `tests/test_legends_and_reminders.py` (5 проверок летописи). **Доказательство:** модель `ServerRecord` (`core/models.py`) читается только в `core/legends.py`; в боте экрана летописи нет.
...
2. **Дом героя и хранилище** (✅ сделано 2026-09-03) — `engine/homestead.py` + `core/homestead.py`: оседлость в безопасных землях, домашний сундук 12–40 ячеек (только у своих дверей, переживает гибель), отдых у очага сильнее костра; тест `tests/test_homestead.py`. Стенд реликвий отложен: реликвий-предметов в игре нет.
