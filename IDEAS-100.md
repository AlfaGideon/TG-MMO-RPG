...
**Статус:** ⬜ не сделано. **Тип пробела:** нет CI.
**Доказательство:** `.github/workflows/` в репозитории отсутствует (`ls .github/workflows` → нет каталога).
**ТЗ:** создать `.github/workflows/ci.yml`: `ubuntu-latest`, `python 3.11`; шаги: (1) `pip install -r requirements.txt pytest pytest-asyncio`; (2) `python3 tests/run_all.py`; (3) `python3 tools/build_bundle.py && python3 tests/test_wiring.py` (бандл не отстал от манифеста).
**Файлы:** `.github/workflows/ci.yml` (новый).
**Критерий приёмки:** push в ветку запускает workflow, все шаги зелёные; поломка любого теста красит CI в красный.
**Что НЕ делать:** не менять логику тестов ради зелёного CI.
...
**Статус:** ⬜ не сделано. **Тип пробела:** рассинхрон бандла ловится косвенно.
**Доказательство:** `tests/test_wiring.py` сверяет `modules.json` с `FALLBACK` в `index.html` и размеры файлов, но не проверяет, что каждый модуль манифеста физически лежит в `webapp/bundle.json`. В этой сессии `engine/karma.py`/`engine/omens.py` попали в бандл только после ручной пересборки.
**ТЗ:** в `tests/test_wiring.py` добавить проверку: распарсить `webapp/bundle.json` (ключ `modules`), сверить множество с `modules.json` и с `FALLBACK.modules`; падать с именем отсутствующего модуля.
**Файлы:** `tests/test_wiring.py`.
**Критерий приёмки:** удалить один модуль из `bundle.json` вручную → тест падает с его именем; вернуть → зелёный.

### № 8. Сквозной pytest-сценарий: карма и знамения через обработчики бота
**Статус:** ⬜ не сделано. **Тип пробела:** нет интеграционного покрытия новых точек входа.
...
### № 9. Смоук-тест бандла: JSON валиден и каждый файл присутствует
**Статус:** ⬜ не сделано. **Тип пробела:** повреждённый бандл обнаруживается только в браузере.
**Доказательство:** `webapp/bundle.json` — единый файл из 85 модулей (`tools/build_bundle.py`); при ошибке склейки панель падает на холодном старте.
**ТЗ:** набор `tests/test_bundle_integrity.py` (скрипт): (1) `json.load` файла; (2) каждый путь из `bundle["modules"]` существует в `modules.json`; (3) контент каждого модуля в бандле равен содержимому файла на диске (сравнение по `hashlib.sha256`).
**Критерий приёмки:** изменить любой `engine/*.py` без пересборки → тест падает и подсказывает «запусти python3 tools/build_bundle.py»; после сборки — зелёный.
...
**Статус:** ⬜ не сделано. **Доказательство:** `core/archaeology.py:find_relic_fragment` — импорт только из `tests/test_lore_and_legends.py`; колонки `relic_fragments`/`treasure_map_coord` (`core/models.py:315-316`) существуют.
**ТЗ:** callback «⛏ Копать» в `bot/handlers/location.py` (экран клетки, как `gather_herbs`); после 5 фрагментов — сообщение с координатами тайника; на клетке тайника — «💰 Выкопать сокровище» (награда — лут из `core/loot.py`), после чего `treasure_map_coord` обнуляется. Realtime-событие `chest_opened` при выкапывании.
**Файлы:** `bot/handlers/location.py`, `bot/keyboards/inline.py`, `tests/test_lore_and_legends.py` (расширить).

### № 46. Инвестиции: вход игроку в боте + выплата дивидендов
**Статус:** ⬜ не сделано. **Доказательство:** `core/investments.py:invest_in_town` — импорт только из `tests/test_economy_and_lunar.py`; модель `TownInvestment` (`core/models.py:1313`).
**ТЗ:** кнопка «🏦 Вклад в лавку» у городского NPC (`talk_npc` в `bot/handlers/location.py`); выплата дивидендов — фоновый цикл в `bot/runner.py` рядом с `_portal_sweep_loop` (раз в сутки, по правилам `core/investments.py`); уведомление игроку о дивидендах; событие `economy_tick` в `core/realtime.py`.
**Файлы:** `bot/handlers/location.py`, `bot/keyboards/inline.py`, `bot/runner.py`, `core/investments.py` (проверить функцию выплаты — если её нет, дописать).

### № 47. Ломбард: вход игроку в боте
**Статус:** ⬜ не сделано. **Доказательство:** `core/pawnshop.py` — импорт только из `tests/test_economy_and_lunar.py`; модель `PawnLoan` (`core/models.py:1329`).
**ТЗ:** NPC-ростовщик (клетка NPC `pawnbroker` или существующий торговец — проверить `core/seed_content.py` на свободный тип NPC); экраны «заложить/выкупить»; просроченные залоги перетекают в чёрный рынок (см. № 48).
**Файлы:** `bot/handlers/location.py` или новый `bot/handlers/pawnshop.py` (+ регистрация роутера в `bot/handlers/__init__.py`), `bot/keyboards/inline.py`.

### № 48. Чёрный рынок: витрина для игроков
**Статус:** ⬜ не сделано. **Доказательство:** `core/blackmarket.py` — импорт только из `tests/test_economy_and_lunar.py`.
**ТЗ:** скрытый NPC-барыга; витрина из `core/blackmarket.py`; цены — как в серверном модуле; после № 47 просроченные залоги попадают в витрину.
**Файлы:** `bot/handlers/location.py` (или отдельный хендлер), `bot/keyboards/inline.py`.

### № 49. Фазы луны: показ игроку в боте
**Статус:** ⬜ не сделано. **Доказательство:** `core/lunar.py` — импорт только из `tests/test_economy_and_lunar.py`.
**ТЗ:** строка фазы луны и её эффектов в экране клетки (`bot/handlers/location.py` `inspect_cell`) и/или в `bot/utils/texts.py`; эффекты фазы уже применяются серверным модулем — ничего не менять в формулах.
**Файлы:** `bot/handlers/location.py`, `bot/utils/texts.py`.

### № 50. Разбор вещей на материалы в боте
**Статус:** ⬜ не сделано. **Доказательство:** `core/salvage.py` — импорт только из `tests/test_economy_and_lunar.py`; крафт в боте есть (`bot/handlers/craft.py`).
**ТЗ:** кнопка «🔧 Разобрать» в карточке предмета (`bot/handlers/inventory.py`) → `core/salvage.py` → материалы в инвентарь крафта.
...
**Статус:** ⬜ не сделано. **Доказательство:** эффект обещан текстом `core/karma.py:karma_status` («защита от фатального удара»), но не реализован ни в одном стеке (первые два эффекта закрыты — № 39–40).
**ТЗ:** одинаково в обоих стеках: если удар сводит HP к 0 и игрок Благочестивый (порог из `karma.py`), один раз за бой HP остаётся 1, в лог пишется «✨ Благословение света спасло тебя»; флаг «защита потрачена» живёт в состоянии боя (`engine/combat.py` `p.combat`, сервер — `bot/handlers/battle.py` `state`).
**Файлы:** `engine/combat.py`, `bot/handlers/battle.py`, `tests/test_karma_omens_engine.py` (расширить).
...
**Статус:** ⬜ не сделано. **Доказательство:** в этой сессии в `engine/models.py:Player` добавлены `bronze`, `silver`, `karma_score`; `Player.from_dict` берёт только известные поля, поэтому старые сохранения открываются — но **надо зафиксировать это тестом**, а не полагаться на «должно работать».
**ТЗ:** тест: `Player.from_dict` старого словаря без новых ключей → поля `bronze/silver/karma_score = 0`; туда-обратно через `to_dict`; сохранение-загрузка через `engine/storage.py` + `webapp/backend.py:MemoryStorage`.
**Файлы:** `tests/test_karma_omens_engine.py` (расширить).
...
| Раздел | Кол-во | Сделано в сессии | Что это за пробелы |
|---|---|---|---|
| 1. Тесты и CI | 10 | № 1, 2, 6, 10 | 12 забытых наборов, ложные провалы, нет CI, флаки |
| 2. Паритет `core/` → `engine/` | 25 | — (карма и знамения перенесены и учтены в Разделе 3) | 25 серверных подсистем без браузерного близнеца |
| 3. Вход игроку в боте | 25 | № 36–44 (9 пунктов) | 21 подсистема без единого вызова из `bot/handlers/` |
| 4. Админка `admin/main.py` | 10 | — | 13 подсистем с нулём упоминаний в админке |
| 5. Панель `webapp/pages/` | 6 | — | страницы Pyodide-панели после переносов Раздела 2 |
| 6. Баги и долги из аудита | 12 | № 77, 78 | долги A/B/C, сироты аукциона, секрет, колбэки, квесты |
| 7. Трёхвалютная экономика | 6 | № 89 | единственный долг реестра паритета |
| 8. Документация | 6 | № 95, 100 | устаревшие цифры и списки |
| **Итого** | **100** | **18** | — |

**Сделано в сессии 2026-08-14: 18 из 100 пунктов** (№ 1, 2, 6, 10, 36–44, 77, 78, 89, 95, 100) + регрессии `tests/test_karma_omens_engine.py` (31 проверка) + полный прогон `python3 tests/run_all.py` (54 набора, все зелёные).
