...
Что делает:
  ...
  * в фоне поднимает Cloudflare Quick Tunnel (core/tunnel.py): панель
    получает публичный HTTPS-адрес, который сам пишется в настройки
    (panel_url) — благодаря этому из бота работает кнопка «🌐 Открыть
    панель» (Telegram Mini App, вход без пароля). Выключается
    ADMIN_TUNNEL=0; если PUBLIC_URL/panel_url заданы вручную — не мешает.
